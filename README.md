...
└── src/
    ...
    │   ├── icons/      # SVG 图标
    │   └── ...
    ├── components/     # 全局通用组件 (DictTag, Editor, Pagination 等)
    ├── directive/      # 自定义指令 (v-hasPermi, v-copy 等)
    ├── layout/         # 页面布局组件
    │   ├── components/ # 布局子组件 (Sidebar, Navbar, AppMain)
    │   └── index.vue   # 布局入口
    ├── plugins/        # 插件注册 (Element, ECharts 等)
    ├── router/         # 路由配置与守卫
    ├── store/          # Pinia 状态管理 (User, App, Permission)
    ├── utils/          # 通用工具函数 (request, auth, validate)
    └── views/          # 页面视图组件
        ├── dashboard/  # 仪表盘
        ├── login/      # 登录页
        ├── system/     # 系统管理 (用户, 角色, 菜单, 部门等)
        ├── monitor/    # 系统监控 (在线用户, 数据监控等)
