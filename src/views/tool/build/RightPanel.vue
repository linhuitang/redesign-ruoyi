  ...
  <div class="right-board">
    ...
    <div class="field-box">
      <el-scrollbar class="right-scrollbar">
        <!-- 组件属性 -->
        <el-form v-show="currentTab === 'field' && showField" label-width="90px" label-position="left" style="padding:20px;">
          ...
          <el-form-item v-if="activeData.align !== undefined && activeData.type === 'flex'" label="垂直排列">
            <el-select v-model="activeData.align" placeholder="请选择垂直排列" :style="{ width: '100%' }">
              <el-option v-for="(item, index) in alignOptions" :key="index" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
          ...
          <el-form-item v-if="activeData.tag === 'el-checkbox-group'" label="至少应选">
            <el-input-number :value="activeData.min" :min="0" placeholder="至少应选" @input="$set(activeData, 'min', $event)" />
          </el-form-item>
          <el-form-item v-if="activeData.tag === 'el-checkbox-group'" label="最多可选">
            <el-input-number :value="activeData.max" :min="0" placeholder="最多可选" @input="$set(activeData, 'max', $event)" />
          </el-form-item>
          ...
          <el-form-item v-if="activeData.icon !== undefined && activeData.tag === 'el-button'" label="按钮图标">
          ...
          <el-form-item
            v-if="activeData.type !== undefined && 'el-date-picker' === activeData.tag || 'el-button' === activeData.tag"
            label="时间类型">
          ...
          <el-form-item v-if="activeData.type !== undefined && 'el-button' === activeData.tag" label="按钮类型">
          ...
          <template v-if="['el-checkbox-group', 'el-radio-group', 'el-select'].indexOf(activeData.tag) > -1">
            ...
            <draggable :list="activeData.options" :animation="340" group="selectItem" handle=".option-drag" item-key="renderId">
             <template #item="{ element, index }">
               <div :key="index" class="select-item">
                ...
                <el-input placeholder="选项值" size="small" :model-value="element.value"
            ...
            <div style="margin-left: 20px;">
              <el-button style="padding-bottom: 0" icon="CirclePlus" link type="primary"
                @click="addSelectItem">
                添加选项
              </el-button>
            </div>
          ...
          <template v-if="['el-cascader'].indexOf(activeData.tag) > -1">
            ...
            <el-tree v-if="activeData.dataType === 'static'" draggable :allow-drop="allowDrop" :allow-drag="allowDrag"
              :data="activeData.options" node-key="id" highlight-current :expand-on-click-node="false">
          ...
          <el-form-item
            v-if="activeData.size !== undefined && (activeData.optionType === 'button' || activeData.border || activeData.tag === 'el-color-picker' || activeData.tag === 'el-button')"
            label="组件尺寸"
          >
              ...
              <template #default="{ node, data }">
                <span class="custom-tree-node">
                  ...
                  <span class="node-icon">
                    <svg-icon :icon-class="data.tagIcon" />
                    <!-- <i :class="data.tagIcon" /> -->
                  </span>
                </span>
              </template>
            </el-tree>
        ...
        <el-form
          ...
          label-width="90px"
          label-position="left" style="padding:20px;"
        >
...
function dateTypeChange(val) {
  if (val === 'datetimerange') {
    props.activeData.format = 'YYYY-MM-DD HH:mm:ss'
    props.activeData['value-format'] = 'YYYY-MM-DD HH:mm:ss'
  }
  if (val === 'daterange') {
    props.activeData.format = 'YYYY-MM-DD'
    props.activeData['value-format'] = 'YYYY-MM-DD'
  }
  if (val === 'datetime') {
    props.activeData.format = 'YYYY-MM-DD HH:mm:ss'
    props.activeData['value-format'] = 'YYYY-MM-DD HH:mm:ss'
...
function setTimeValue(val, type) {
  const valueFormat = type === 'week' ? 'gggg-ww' : 'YYYY-MM-DD'
...
function addTreeItem() {
  ++props.activeData.options.length
  props.activeData.options.push({
    label: '新选项',
    value: '新选项',
    children: []
...
// 确保 loaded options 有 renderId
watchEffect(() => {
  if (props.activeData && props.activeData.options && Array.isArray(props.activeData.options)) {
    props.activeData.options.forEach(item => {
      if (!item.renderId) item.renderId = +new Date() + Math.random()
    })
  }
})
</script>
